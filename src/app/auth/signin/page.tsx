'use client'

import { signIn } from 'next-auth/react'
import { useState, useEffect } from 'react'

interface Provider { id: string; name: string }

export default function SignInPage() {
  const [email,     setEmail]     = useState('')
  const [sending,   setSending]   = useState(false)
  const [sent,      setSent]      = useState(false)
  const [providers, setProviders] = useState<Record<string, Provider>>({})

  useEffect(() => {
    fetch('/api/auth/providers')
      .then(r => r.json())
      .then(setProviders)
      .catch(() => {})
  }, [])

  const hasGitHub = 'github' in providers
  const hasEmail  = 'resend' in providers

  async function handleMagicLink(e: React.FormEvent) {
    e.preventDefault()
    setSending(true)
    await signIn('resend', { email, redirect: false, callbackUrl: '/' })
    setSending(false)
    setSent(true)
  }

  return (
    <main style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:24, background:'#0f1117' }}>
      <div style={{ width:'100%', maxWidth:400 }}>

        <div style={{ textAlign:'center', marginBottom:32 }}>
          <h1 style={{ fontSize:28, fontWeight:700, color:'#fff' }}>💳 Finance Analyzer</h1>
          <p style={{ color:'#94a3b8', marginTop:8 }}>Sign in to unlock more features</p>
        </div>

        {/* OAuth buttons */}
        <div style={{ display:'flex', flexDirection:'column', gap:12, marginBottom: hasEmail ? 24 : 0 }}>
          <button onClick={() => signIn('google', { callbackUrl: '/' })} style={oauthBtn('#fff', '#0f1117')}>
            <GoogleIcon /> Continue with Google
          </button>
          {hasGitHub && (
            <button onClick={() => signIn('github', { callbackUrl: '/' })} style={oauthBtn('#24292e', '#fff')}>
              <GitHubIcon /> Continue with GitHub
            </button>
          )}
        </div>

        {/* Magic link — only shown if Resend is configured */}
        {hasEmail && (
          <>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
              <div style={{ flex:1, height:1, background:'#334155' }} />
              <span style={{ color:'#64748b', fontSize:13 }}>or use email</span>
              <div style={{ flex:1, height:1, background:'#334155' }} />
            </div>

            {!sent ? (
              <form onSubmit={handleMagicLink} style={{ display:'flex', flexDirection:'column', gap:12 }}>
                <input
                  type="email" required value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="your@email.com"
                  style={{ padding:'12px 16px', borderRadius:10, border:'1px solid #334155', background:'#1e293b', color:'#fff', fontSize:15, outline:'none' }}
                />
                <button type="submit" disabled={sending} style={primaryBtn}>
                  {sending ? 'Sending...' : 'Send magic link'}
                </button>
              </form>
            ) : (
              <div style={{ textAlign:'center', padding:24, background:'rgba(99,102,241,0.1)', border:'1px solid #6366f1', borderRadius:12 }}>
                <p style={{ color:'#a5b4fc', fontSize:16 }}>✉️ Check your email!</p>
                <p style={{ color:'#64748b', fontSize:13, marginTop:8 }}>
                  We sent a magic link to <strong style={{ color:'#94a3b8' }}>{email}</strong>
                </p>
              </div>
            )}
          </>
        )}

        <p style={{ textAlign:'center', color:'#475569', fontSize:12, marginTop:24 }}>
          By signing in you agree to our{' '}
          <a href="/terms" style={{ color:'#6366f1' }}>Terms</a> and{' '}
          <a href="/privacy" style={{ color:'#6366f1' }}>Privacy Policy</a>
        </p>

        <p style={{ textAlign:'center', marginTop:16 }}>
          <a href="/" style={{ color:'#64748b', fontSize:13 }}>← Back to home</a>
        </p>
      </div>
    </main>
  )
}

const primaryBtn: React.CSSProperties = {
  padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer',
  background:'#6366f1', color:'#fff', fontSize:15, fontWeight:600,
}

function oauthBtn(bg: string, color: string): React.CSSProperties {
  return {
    display:'flex', alignItems:'center', justifyContent:'center', gap:10,
    padding:'12px 24px', borderRadius:10, border:'1px solid #334155',
    background: bg, color, fontSize:14, fontWeight:500, cursor:'pointer',
  }
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  )
}

function GitHubIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/>
    </svg>
  )
}
